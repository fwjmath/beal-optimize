...
//#define MODP1(X) ((X) % 3491888400ULL) //highly composite number
#define MODP1(X) ((X) % 4227022800ULL) //highly abundant number
#define MODP2(X) ((X) % 4294967087ULL) //safe prime
...
unsigned long long h1cnt, h2cnt;

void build_hash(){
	p1hash.clear();
	p2hash.clear();
	for(unsigned int z=1; z<=BASE_MAX; z++){
		unsigned long long zp1=MODP1(MODP1((unsigned long long)z*z)*z), zp2=MODP2(MODP2((unsigned long long)z*z)*z);
		p1hash.insert(zp1);
		p2hash.insert(zp2);
